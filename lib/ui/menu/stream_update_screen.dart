...
class StreamUpdateScreen extends StatefulWidget {
  @override
  StreamUpdateScreenState createState() => StreamUpdateScreenState();
}

class StreamUpdateScreenState extends State<StreamUpdateScreen> {
                  ...
                  children: [
                    ...
                    FutureBuilder(
                        future: streamSelectionRepository.fetchStreams(
                            boardName:
                                updatedAppUser.educationBoard!.toLowerCase(),
                            classID: updatedAppUser.classID),
                        builder: (context, streams) {
                          if (streams.connectionState == ConnectionState.none &&
                              streams.hasData == null) {
                            return Container();
                          } else if (streams.hasData) {
                            List<StreamsModel>? _streamsList = streams.data as List<StreamsModel>?;
                    ...
                    OnBoardingBottomButton(
                      buttonText: "Confirm",
                      buttonColor:
                          selectedStream!.isNotEmpty ? 0xFF0077FF : 0xFFDEDEDE,
                      onPressed: () async {
                        if (selectedStream!.isNotEmpty) {

                          updatedAppUser.stream = selectedStream;
